...
func (h Hooks) PostTxProcessing(
	...
	for _, log := range receipt.Logs {
		// section for ica stuff

		//address := log.Address
		//toAddr := sdk.AccAddress(address.Bytes())

		//err := h.k.IntertxKeeper.RegisterInterchainAccount(ctx, "connection-0", toAddr.String())
		//if err != nil {
		//	h.k.Logger(ctx).Error(err.Error())
		//}

		// here we define logic like transfer and all that
		//var msg sdk.Msg
		//
		//h.k.IntertxKeeper.SubmitTxICA(ctx, "connection-0", toAddr.String(), msg)
		...
		//if event.Name != types.ICACALLEvent {
		...
		//icaevent, err := icacall.Unpack(event.Name, log.Data)
		////h.k.Logger(ctx).Debug(icaevent)
		//fmt.Println(icaevent)
